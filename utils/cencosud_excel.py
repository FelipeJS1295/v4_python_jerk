    ...
    for index, row in df.iterrows():
        ...
        try:
            ...
            venta = {
                ...
                "rut_documento": row.get('rut_cliente', row.iloc[2]),
                "email": row.get('mail_cliente', row.iloc[3]),
                "telefono": row.get('telefono_contacto', row.iloc[4]),
                "fecha_compra": convertir_fecha(row.get('fecha_compra', row.iloc[5])),
                "fecha_entrega": convertir_fecha(row.get('fecha_entrega_comprometida', row.iloc[6])),
                "producto": row.get('descripcion_producto', row.iloc[8]),
                "precio_cliente": limpiar_numero(row.get('precio_unitario', row.iloc[10])),
                "comuna": row.get('comuna_despacho', row.iloc[12]),
                "direccion": row.get('direccion_despacho', row.iloc[13]),
                "sku": row.get('sku_paris', row.iloc[16]),
                "estado": "nueva",
                "unidades": 1,
